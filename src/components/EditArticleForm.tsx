...
export default function EditArticleForm({ article }: { article: Article }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMsg("Articol actualizat cu succes!");
    } catch (err: any) {
      setErrorMsg(err.message || "Eroare la actualizare");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-4">
      <form onSubmit={handleSubmit} className="space-y-4 border p-4 rounded-lg">
        <h2 className="text-xl font-semibold mb-2">Editează articol</h2>
        ...
        <button
          ...
          onClick={() => navigate("/admin/articole")}
        >
          Înapoi la listă
        </button>
      </form>
